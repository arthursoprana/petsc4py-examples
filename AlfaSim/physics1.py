...
#         Uc = (1 / α[:-1]) * 2.0 * (αf[1:] * U[1:] * αf[:-1] * U[:-1]) / (αf[1:] * U[1:] + αf[:-1] * U[:-1])
        ...
        me = ((0.5 - β[1:  ]) * ρ[1:  ] + (0.5 + β[1:  ]) *  ρ[ :-1]) * US[1:  ] * A
        mw = ((0.5 - β[ :-1]) * ρ[ :-1] + (0.5 + β[ :-1]) * ρρ[ :-2]) * US[ :-1] * A  
...
#         mwc = 0.5 * (mw[1:] + mw[:-1])        
        ...
        f[1:-1, phase] += \
            ...
            + mec * ((0.5 - β[1:  ]) * US[2:  ] + (0.5 + β[1:  ]) * US[1:-1]) \
            - mwc * ((0.5 - β[ :-1]) * US[1:-1] + (0.5 + β[ :-1]) * US[ :-2]) \
        ...
        f[-1, phase] += \
            ...
            + ρ[-2] * U[-1]  * A * US[-1] \
            - ρ[-2] * Uc[-1] * A * ((0.5 - β[-1]) * US[-1] + (0.5 + β[-1]) * US[-2]) \
        ...
        # boundaries            
        # Momentum     
        f[0,phase] = -(Mpresc[phase] - ρρ[0] * US[0] * A)
